...
setInterval(function updateTodos() {
  ...
  todoService.find({}, function(error, allTodos) {
    ...
    if(!todo) {
      todoService.create({ text: 'Check out Feathers!', completed: false, server: true }, {}, noop);
    } else if(todo.text !== text) {
      todoService.update(todo.id, _.extend(todo, { completed: false, text: text }), {}, noop);
    } else if(!todo.completed) {
      todoService.update(todo.id, _.extend(todo, { completed: true }), {}, noop);
    } else {
